...
int main()
{
	...
	for (int row = 0; row < numRows; row++) //0<=row<1498의 조건을 만족하면 반복문 실행. 1498번 반복.
	{
		...
		{
			//data1D[row*numCols+col] : data1D의 row*numCols+col번지 주소의 데이터 값. datatype이 char이므로, 문자로 출력됨.
			if (data1D[row * numCols + col] > 128)
				data1D[row * numCols + col] = 255; //data1D[row*numCols+col] 값이 128보다 큰 경우 값을 255로 바꿔줌. (흰)
			else
				data1D[row * numCols + col] = 0; // //data1D[row*numCols+col] 값이 128보다 작거나 같은 경우 값을 0으로 바꿔줌. (검)
		}
	}
	cout << "이진화 완료." << endl;
	...
	outFile.open("binarization.pgm", ios::binary); //논리적 파일(outFile)과 물리적 파일(binarization.pgm)을 연결해줌. 
