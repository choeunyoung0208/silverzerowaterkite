...
figure, bar(binLocations, make_histeq(f, a(1)-1, counts)); title('평활화된 영상'); %평활화된 영상 f의 밝기분포 히스토그램을 막대 그래프 형태로 출력
...
gray_level=0:1:num; %가질 수 있는 그레이 레벨 범위는 0~num(최대 그레이 레벨)까지의 정수를 가지므로, colon operator를 사용해 가질 수 있는 그레이 레벨 범위를 생성.
...
%직접 만든 함수를 사용해서 평활화한 결과 그래프와 이전에 histeq명령어를 사용해서 평활화한 결과 그래프를 비교해보니 똑같은 것 같기도 하고.. 약간 다른 것 같기도 합니다...



%교수님,,,이걸 보실진 모르겠지만... 혹시나 보시게 된다면... 제가 종강하고나서 시도때도없이 계속먹었더니..턱에 무리가 갔는지... 턱이 아프더라구요.......그래서 내일 병원을 가야될 것 같아서...내일하려고 했던것들 미리 올립니다..ㅠㅠ
